...
const Grades = () => {
  ...
  const fetchGrades = async () => {
    try {
      ...
      //  .filter(([student, grade]) => grade !== null && !isNaN(grade))
      //  .map(([student, grade]) => ({
      //  student: `"${String(student).trim().replace(/\0/g, '')}"`,
      //  grade: grade,
      // }));
  ...
  const addStudent = async () => {
    try {
      const response = await fetch(API_url, {
        ...
        body: JSON.stringify({ student: addName.replace("/", ""), grade: parseFloat(addGrade)}),
      });
